...
class BaseLearner(object):
    def __init__(self, logger, config):
        ...
        self._text_inputs = None # CLIP的文本输入模板
    ...
    def clip_text_input(self, classes_name, class_order, prompts = None, prompt_num = 0, use_dataset = 'cifar100'):
        ...
        # clip_model.eval()
        new_classes_name = list(map(lambda x: classes_name[x], class_order))
        if not prompts:  # 默认人工模板， a photo of a CLS
            if use_dataset == 'cifar100':
                self._text_inputs = torch.cat([clip.tokenize(f"a bad photo of a {c}") for c in classes_name]) #生成文字描述
            else:
                self._text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in classes_name]) #生成文字描述
        else:
            assert prompt_num != 0, "Template num is zero!"
            text_inputs = []
            for idx,class_name in enumerate(new_classes_name):
                assert class_name in prompts, "Class_name not in prompt_json!"
                cls_prompt = sample(prompts[class_name], prompt_num)
                text_tokenize = torch.cat([clip.tokenize(c) for c in cls_prompt])  # [prompt_num, 77]
                text_inputs.append(text_tokenize)
                # text_inputs[idx] = text_tokenize
            self._text_inputs = torch.stack(text_inputs,dim=0) # [classes_num, prompt_num, 77]
            # self._text_features = clip_model.encode_text(self._text_inputs)

        if use_dataset == 'cifar100':
            self._zero_shot_text_inputs = torch.cat([clip.tokenize(f"a bad photo of a {c}") for c in classes_name]) #生成文字描述
        else:
            self._zero_shot_text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in classes_name]) #生成文字描述
        self._logger.info(f"Template nums is {len(classes_name)} * {prompt_num} = {self._text_inputs.shape[0]}")