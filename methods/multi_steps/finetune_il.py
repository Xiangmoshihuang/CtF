...
class Finetune_IL(BaseLearner):
    def __init__(self, logger, config):
        ...
        self.clip_metric_curve = []
        self.zs_metric_curve = []
        self.nme_metric_curve = []
        self.topk_metric_curve = []
        # for openset test
        self.cnn_auc_curve = []
        self.nme_auc_curve = []
        ...
        self.manual_temp = config.manual_temp
        ...
        self._prompt_num = config.prompt_num
        if self._backbone == "resnet18_rep":
            self.ssre_args = {
                'dataset': config.dataset,
                'mode': config.ssre_mode,
    ...
    def prepare_task_data(self, data_manager):
        ...
        if self._cur_task == 0 and self._config.method in('ssre', 'fetril'): 
            self._train_dataset = data_manager.get_dataset(indices=np.arange(self._known_classes, self._total_classes),
                    source='train', mode='ssre')
        elif self._cur_task > 0 and self._memory_bank != None:
            self._train_dataset = data_manager.get_dataset(indices=np.arange(self._known_classes, self._total_classes), 
        ...
        self._CLIP_test_dataset = data_manager.get_dataset(indices=np.arange(0, self._total_classes), source='test', mode='clip')
        ...
        self._CLIP_test_loader = DataLoader(self._CLIP_test_dataset, batch_size=self._batch_size, shuffle=False, num_workers=self._num_workers)
        ...
        if self._text_inputs == None:
            if self.manual_temp:
                self.clip_text_input(data_manager._classes_name, data_manager._class_order, use_dataset=data_manager.dataset_name)
            else:
                self.clip_text_input(data_manager._classes_name, data_manager._class_order, data_manager._prompts, prompt_num=self._prompt_num, use_dataset=data_manager.dataset_name)

    def prepare_model(self, checkpoint=None):
        if self._network == None:
            ssre_arg = self.ssre_args if self._config.backbone == 'resnet18_rep' else None
            self._network = IncrementalNet(self._logger, self._config.backbone, self._config.pretrained, self._config.pretrain_path, MLP_projector=self._config.MLP_projector, backbone_config=ssre_arg)
        
        self._network.update_fc(self._total_classes)
        if checkpoint is not None:
            self._network.load_state_dict(checkpoint['state_dict'], strict=False)
    ...
    def _epoch_test(self, model, test_loader, clip_test_loader, ret_task_acc=False, ret_clip_acc=False, ret_pred_target=False, task_begin=None, task_end=None, task_id=None):
        cnn_correct, cnn_task_correct, total, task_total = 0, 0, 0, 0
        clip_correct, clip_task_correct = 0, 0
        zs_correct, zs_task_correct = 0, 0  # clip_zs : zero shot
        cnn_pred_all, clip_pred_all, clip_zs_pred_all, cnn_topk_pred_all, nme_pred_all, target_all, features_all = [], [], [], [], [], [], []
        logits_all, clip_simi_all = [], []
        ...
        if ret_clip_acc:
            ...
            clip_model.eval()
        loader = zip(test_loader, clip_test_loader) if clip_test_loader is not None else test_loader
        for item in loader:
            clip_inputs = None
            if len(item) == 3:
                _, inputs, targets = item
                inputs, targets = inputs.cuda(), targets.cuda()
                clip_inputs = inputs
            else:
                item1, item2 = item
                _, inputs, targets = item1
                _, clip_inputs, __ = item2
                inputs, clip_inputs, targets = inputs.cuda(), clip_inputs.cuda(), targets.cuda()
            
            
            logits, feature_outputs = model(inputs)
            bs = logits.shape[0]
            if ret_clip_acc:
                if self._memory_bank != None and self._apply_nme:  # 例如icarl等nme方法时，使用nme的top-k
                    _, dist = self._memory_bank.KNN_classify(task_begin, task_end, feature_outputs['features'], ret_logits=True)
                    predicts_topk = torch.topk(dist, k=self.topk, dim=1, largest=False, sorted=True)[1] # [bs, topk]
                # predicts_topk = torch.topk(dist, k=self.topk, dim=1)[1] # [bs, topk]
                else:
                    predicts_topk = torch.topk(logits[:, :task_end], k=self.topk, dim=1, largest=True, sorted=True)[1] # [bs, topk]
                # CLIP interference
                with torch.no_grad():
                    if not self.prompts:
                        text_inputs = torch.cat([self._text_inputs[i] for i in predicts_topk]).cuda() # 从template中挑选出top5对应的文本输入 # [320,77]
                        image_features = clip_model.encode_image(clip_inputs)  # [bs, 512]
                        text_features = clip_model.encode_text(text_inputs) # 【bs*topk, 512】
                        image_features = image_features.reshape((bs,1,-1)) # [bs, 1, 512]
                        text_features = text_features.reshape((bs,self.topk,-1))   # [bs, 5, 512]
                        image_features /= image_features.norm(dim=-1, keepdim=True)
                        text_features /= text_features.norm(dim=-1, keepdim=True)
                    else:
                        text_inputs = torch.cat([self._text_inputs[key] for key in predicts_topk]).cuda() # [bs*topk,prompt_num,77]
                        text_inputs = text_inputs.reshape((-1,77))  # [bs*topk*prompt_num,77]
                        image_features = clip_model.encode_image(clip_inputs)  # [bs, 512]
                        text_features = clip_model.encode_text(text_inputs) # 【bs*topk*prompt_num, 512】
                        image_features = image_features.reshape((bs,1,-1)) # [bs, 1, 512]
                        image_features /= image_features.norm(dim=-1, keepdim=True)
                        # 集成方式1， 取相似度的平均值
                        # text_features = text_features.reshape((bs,self.topk*self.prompt_num,-1))   # [bs, topk*prompt_num, 512]
                        # text_features /= text_features.norm(dim=-1, keepdim=True)

                        # 集成方式2， 取Text Embedding的平均值
                        text_features /= text_features.norm(dim=-1, keepdim=True)
                        text_features = text_features.reshape((bs, self.topk, self.prompt_num, -1)).mean(dim=-2) # [bs, topk, 512]
                        text_features /= text_features.norm(dim=-1, keepdim=True)
                    
                    similarity = (image_features @ text_features.transpose(-1,-2))
                    similarity = torch.squeeze(similarity).softmax(dim=-1) # 最后一个item的bs为1，squeeze后，shape为[1]     
                    values_top1, indices_top1 = similarity.topk(1) # indices_top1 的shape为[1]， 而predicts_topk的shape为[1,5]
                    if bs == 1:
                        indices_top1 = torch.reshape(indices_top1, (1,-1))
                        clip_top1 = torch.gather(predicts_topk, 1, indices_top1)
                        clip_top1 = clip_top1.reshape(-1)
                    else:
                        clip_top1 = torch.gather(predicts_topk, 1, indices_top1).squeeze()

                    # CLIP zero shot:
                    zs_text_inputs = torch.cat([self._zero_shot_text_inputs[:self._total_classes]]).cuda() # 从template中挑选出top5对应的文本输入 # [320,77]
                    zs_image_features = clip_model.encode_image(clip_inputs)  # [bs, 512]
                    zs_text_features = clip_model.encode_text(zs_text_inputs) # 【total_classes, 512】
                    zs_image_features /= zs_image_features.norm(dim=-1, keepdim=True)
                    zs_text_features /= zs_text_features.norm(dim=-1, keepdim=True)
                    logits_per_image = zs_image_features @ zs_text_features.t()

                    zs_scores, zs_preds = torch.max(torch.softmax(logits_per_image, dim=-1), dim=-1)
            if self._incre_type == 'cil':
            ...
            if ret_pred_target:
                ...
                clip_pred_all.append(tensor2numpy(clip_top1))
                clip_zs_pred_all.append(tensor2numpy(zs_preds))
                # clip_simi_all.append(tensor2numpy(similarity))
                logits_all.append(tensor2numpy(logits))
            ...
            if ret_clip_acc:
                # top5 acc
                cnn_topk_pred_all.append(tensor2numpy(predicts_topk.cpu()))
                clip_correct += clip_top1.eq(targets).cpu().sum()
                zs_correct += zs_preds.eq(targets).cpu().sum()

        if ret_pred_target:
            cnn_pred_all = np.concatenate(cnn_pred_all)
            clip_pred_all = np.concatenate(clip_pred_all)
            clip_zs_pred_all = np.concatenate(clip_zs_pred_all)
            logits_all = np.concatenate(logits_all)
            # clip_simi_all = np.concatenate(clip_pred_all)
            ...
            if ret_clip_acc:
                clip_acc = np.around(tensor2numpy(clip_correct)*100 / total, decimals=2)
                zs_acc = np.around(tensor2numpy(zs_correct)*100 / total, decimals=2)
                #top5 acc
                cnn_topk_pred_all = np.concatenate(cnn_topk_pred_all)
                topk_acc = np.around(
                (cnn_topk_pred_all.T == np.tile(target_all, (self.topk, 1))).sum() * 100 / len(target_all),
                decimals=2,
                )
                return cnn_pred_all, clip_pred_all, logits_all, None, nme_pred_all, cnn_max_scores_all, nme_max_scores_all, target_all, features_all, clip_acc, zs_acc, topk_acc, cnn_topk_pred_all
            else:
            ...
            if ret_task_acc:
                ...
                if ret_clip_acc:
                    clip_task_acc = np.around(tensor2numpy(clip_task_correct)*100 / task_total, decimals=2)
                    return test_acc, test_task_acc, clip_acc, clip_task_acc, topk_acc
                else:
    ...
    def _train_model(self, model, train_loader, test_loader, optimizer, scheduler, task_id=None, epochs=100, note=''):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 15 == 0:
                test_acc, task_test_acc = self._epoch_test(model, test_loader, clip_test_loader=None, ret_task_acc=True, ret_clip_acc=False,
                                                    task_begin=task_begin, task_end=task_end, task_id=task_id)
    ...
    def eval_task(self):        
        ...
        if self._is_openset_test:
            ...
            if self._config.backbone in ("cosine_resnet32", "resnet32", "resnet18_rep") :
                clip_test_loader = self._CLIP_test_loader
        if self._incre_type == 'cil':
            cnn_pred, clip_pred, logits_pred, clip_simi, nme_pred, cnn_pred_score, nme_pred_score, y_true, features, clip_acc, zs_acc, topk_acc, topk_idx = self.get_cil_pred_target(self._network, test_loader, clip_test_loader)
            # test_acc, test_task_acc, clip_acc, clip_task_acc, topk_acc = self._epoch_test(self._network, test_loader, clip_test_loader, ret_task_acc=True, ret_clip_acc=True, task_begin=0, 
        ...
        if self._save_pred_record:
            self.save_predict_records(cnn_pred, clip_pred, logits_pred, clip_simi, cnn_pred_score, nme_pred, nme_pred_score, y_true, features, topk_idx)
        ...
        self.clip_metric_curve.append(clip_acc)
        self.zs_metric_curve.append(zs_acc)
        self.topk_metric_curve.append(topk_acc)
        ...
        self._logger.info("CLIP : {} curve of all task is [\t".format(self._eval_metric) + 
            ("{:2.2f}\t"*len(self.clip_metric_curve)).format(*self.clip_metric_curve) + ']')
        self._logger.info("ZS : {} curve of all task is [\t".format(self._eval_metric) + 
            ("{:2.2f}\t"*len(self.zs_metric_curve)).format(*self.zs_metric_curve) + ']')
        self._logger.info("Top-{} : {} curve of all task is [\t".format(self.topk, self._eval_metric) + 
            ("{:2.2f}\t"*len(self.topk_metric_curve)).format(*self.topk_metric_curve) + ']')
        ...
        self._logger.info("CLIP : Average (all task) {} of all stages: {:.2f}".format(self._eval_metric, np.mean(self.clip_metric_curve)))
    ...
    def save_predict_records(self, cnn_pred, clip_pred, logits_pred, clip_simi, cnn_pred_scores, nme_pred, nme_pred_scores, targets, features, topk_idx):
        record_dict = {}        
        ...
        record_dict['clip_pred'] = clip_pred
        record_dict['targets'] = targets
        record_dict['features'] = features
        record_dict['logits_pred'] = logits_pred
        record_dict['topk_idx'] = topk_idx
        record_dict['clip_simi'] = clip_simi

        filename = 'pred_record_seed{}_task{}.npy'.format(self._seed, self._cur_task)
        np.save(join(self._logdir, filename), record_dict)

    def get_cil_pred_target(self, model, test_loader, clip_test_loader):
        return self._epoch_test(model, test_loader, clip_test_loader, ret_pred_target=True, ret_clip_acc=True, task_begin=0, 
                    task_end=self._total_classes, task_id=self._cur_task)
    ...
    def _get_optimizer(self, params, config, is_init:bool):
        ...
        if is_init:
            ...
            elif config.opt_type == None:
                self._logger.info('Applying None optimizer!')
            else:
                self._logger.info('Applying adamw: lr={}, weight_decay={}'.format(self._init_lrate, self._init_weight_decay))
        else:
            if config.opt_type == 'sgd':
    ...
    def _get_scheduler(self, optimizer, config, is_init:bool):
        ...
        if is_init:
            if config.scheduler == 'multi_step':
                ...
                self._logger.info('Applying None scheduler!')
                scheduler = None
            else: 
