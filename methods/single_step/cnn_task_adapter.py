...
from backbone.adapter_cl_net import CNN_Task_Adapter_Net
...
class CNN_Task_Adapter(Finetune_normal):
    ...
    def prepare_model(self, checkpoint=None):
        if self._network == None:
            ...
            self._logger.info('Created Task-specific adapter net!')
    ...
    def _epoch_test(self, model, test_loader, ret_pred_target=False):
        ...
        cnn_max_scores_all, features_all = [], []
        model.eval()
        for _, inputs, targets in test_loader:
            ...
            if ret_pred_target:
                ...
                cnn_correct += cnn_preds.cpu().eq(targets).sum()
                total += len(targets)
