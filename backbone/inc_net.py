...
from backbone.modified_represnet import resnet18_rep
...
def get_backbone(logger, backbone_type, pretrained=False, pretrain_path=None, normed=False, backbone_config=None) -> nn.Module:
    ...
    if name == 'resnet32':
        ...
        net = resnet18_rep(pretrained=pretrained, args=backbone_config)
        ...
        net = clip_model.visual
    elif name == 'resnet101_clip':
        clip_model = load_clip_to_cpu('RN101')
        net = clip_model.visual
    elif name == 'vit_base_patch16_224_clip':
        clip_model = load_clip_to_cpu('ViT-B/16')
        net = clip_model.visual
    elif name == 'vit_base_patch32_224_clip':
...
def load_clip_to_cpu(backbone_type):
    from backbone.clip import clip
    url = clip._MODELS[backbone_type]
...
class IncrementalNet(nn.Module):
    ...
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names: Iterable[str]=[], MLP_projector=False, backbone_config=None):
        ...
        self._logger = logger
        self.feature_extractor = get_backbone(self._logger, backbone_type, pretrained, pretrain_path, backbone_config=backbone_config)
        self._layer_names = layer_names
        if ('resnet' in backbone_type or 'vit' in backbone_type) and 'clip' in backbone_type:
            self._feature_dim = self.feature_extractor.output_dim
        elif backbone_type == "resnet18_rep":
            self._feature_dim = self.feature_extractor.out_dim
