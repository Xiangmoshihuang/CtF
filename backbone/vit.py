##### Notice:
# This file is copied from timm.models.vision_transformer
# we did a little change in the forward fucntion of Class Block, ResPostBlock and ParallelBlock.
# codes like "x = x + f(x)" is splited into "x1 = f(x)" and "x2 = x + x1" for convenience to apply hooks
#####
...
class Attention(nn.Module):
    ...
    def forward(self, x, prompt=None):
        ...
        if prompt is not None:
            ...
            # k, v shape: [B, num_heads, token_num+prompt_num, token_dim / num_heads]
        
        attn = (q @ k.transpose(-2, -1)) * self.scale
        # attn shape: [B, num_heads, token_num, token_num+prompt_num]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        # x-transpose shape: [B, num_heads, token_num, token_dim / num_heads] => [B, token_num, num_heads, token_dim / num_heads]
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, prompt=None):
        x1 = self.drop_path1(self.ls1(self.attn(self.norm1(x), prompt=prompt)))
        if x.shape[0] == x1.shape[0]:
            x = x + x1
        elif x1.shape[0] % x.shape[0] == 0:
            x = x.repeat(x1.shape[0] // x.shape[0], 1, 1) + x1
        else:
            raise NotImplementedError('Batch size of input tensor {} did not match {}!'.format(x.shape[0], x1.shape[0]))
        
        x2 = self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        if x.shape[0] == x2.shape[0]:
            x = x + x2
        elif x2.shape[0] % x.shape[0] == 0:
            x = x.repeat(x2.shape[0] // x.shape[0], 1, 1) + x2
        else:
            raise NotImplementedError('Batch size of input tensor {} did not match {}!'.format(x.shape[0], x2.shape[0]))
        return x
...
class ResPostBlock(nn.Module):
    ...
    def forward(self, x, prompt=None):
        x1 = self.drop_path1(self.norm1(self.attn(x, prompt=prompt)))
        if x.shape[0] == x1.shape[0]:
            x = x + x1
        elif x1.shape[0] % x.shape[0] == 0:
            x = x.repeat(x1.shape[0] // x.shape[0], 1, 1) + x1
        ...
        x2 = self.drop_path2(self.norm2(self.mlp(x)))
        if x.shape[0] == x2.shape[0]:
            x = x + x2
        elif x2.shape[0] % x.shape[0] == 0:
            x = x.repeat(x2.shape[0] // x.shape[0], 1, 1) + x2
...
class ParallelBlock(nn.Module):
    ...
    @torch.jit.ignore
    def _forward(self, x):
        x1 = sum(attn(x) for attn in self.attns)
        if x.shape[0] == x1.shape[0]:
            x = x + x1
        elif x1.shape[0] % x.shape[0] == 0:
            x = x.repeat(x1.shape[0] // x.shape[0], 1, 1) + x1
        ...
        x2 = sum(ffn(x) for ffn in self.ffns)
        if x.shape[0] == x2.shape[0]:
            x = x + x2
        elif x2.shape[0] % x.shape[0] == 0:
            x = x.repeat(x2.shape[0] // x.shape[0], 1, 1) + x2
    ...
    def forward(self, x, prompt=None, q=None, train=False, task_id=None):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if prompt is not None:
                ...
                if prompt.prompt_mode == 'prefix_tune' and prompt_tokens is not None:
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        ...
        pretrained_custom_load='npz' in pretrained_cfg['url'],    # pretrained_cfg['url']
        **kwargs)
    return model
