...
class Special_Adapter_v2(nn.Module):
    def __init__(self, in_planes:int, kernel_size:int, use_alpha=True):
        super().__init__()
        self.in_planes = in_planes
        self.conv = nn.Conv2d(in_planes, in_planes, kernel_size=kernel_size, padding=1)
...
class Channel_Adapter_v1(nn.Module):
    """
    x -> conv 1x1 -> bn -> relu ->
    """
    def __init__(self, in_planes:int, use_alpha=True):
        super().__init__()
        self.in_planes = in_planes
...
class Channel_Adapter_v2(nn.Module):
    """
    x -> bn -> relu -> conv 1x1 -> 
    """
    def __init__(self, in_planes:int, use_alpha=True):
...
class ViT_Adapter(nn.Module):
    ...
    x -> W_down -> GELU -> W_up -> 
    """
    def __init__(self, in_features:int, hidden_dim:int, use_alpha=True):
        super().__init__()
...
class CNN_Task_Adapter_Net(IncrementalNet):
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names:Iterable[str]=[], mode=None, hidden_dim=8):
        '''
        ...
        for layer_id in layer_names:
            ...
            layer.register_forward_pre_hook(self.forward_adapter(layer_id))

    def forward_adapter(self, layer_id: str) -> Callable:
        def hook(module, input):
            ...
            if not hasattr(self, layer_id+'_adapter'):
                if 'conv5x5' in self.mode:
                    kernel_size = 5
                    self._logger.info('Applying 5x5 conv in adapter')
                elif 'conv7x7' in self.mode:
                    kernel_size = 7
                    self._logger.info('Applying 7x7 conv in adapter')
                else:
                    kernel_size = 3
                    self._logger.info('Applying 3x3 conv in adapter')

                if 'special_adapter_v1' in self.mode:
                    self.register_module(layer_id+'_adapter', Special_Adapter_v1(c, c, kernel_size, use_alpha='use_alpha' in self.mode).cuda())
                    self._logger.info('Created task-specific adapter (special v1) before layer-{}: {}=>{}=>{}'.format(layer_id, c, c, c))
                elif 'special_adapter_v2' in self.mode:
                    self.register_module(layer_id+'_adapter', Special_Adapter_v2(c, kernel_size, use_alpha='use_alpha' in self.mode).cuda())
                    self._logger.info('Created task-specific adapter (special v2) before layer-{}: {}=>{}=>{}'.format(layer_id, c, c, c))
                elif 'channel_adapter_v1' in self.mode:
                    self.register_module(layer_id+'_adapter', Channel_Adapter_v1(c, use_alpha='use_alpha' in self.mode).cuda())
                    self._logger.info('Created task-specific adapter (channel v1) before layer-{}: {}=>{}'.format(layer_id, c, c))
                elif 'channel_adapter_v2' in self.mode:
                    self.register_module(layer_id+'_adapter', Channel_Adapter_v2(c, use_alpha='use_alpha' in self.mode).cuda())
                    self._logger.info('Created task-specific adapter (channel v2) before layer-{}: {}=>{}'.format(layer_id, c, c))
                elif 'vit_adapter' in self.mode:
                    self.register_module(layer_id+'_adapter', ViT_Adapter(c, self.hidden_dim, use_alpha='use_alpha' in self.mode).cuda())
                    self._logger.info('Created task-specific adapter (vit adapter) before layer-{}: {}=>{}=>{}'.format(layer_id, c, self.hidden_dim, c))
                else:
                    raise ValueError("Unknown adapter's type in mode: {}".format(self.mode))
            return (getattr(self, layer_id+'_adapter')(input) + input,)
        return hook
...
class CNN_Adapter_Net_CIL_V2(IncrementalNet):
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names:Iterable[str]=[], mode=None, addition_dim=1, hidden_dim=8):
        '''
        ...
        def hook(module, input):
            ...
            if len(getattr(self, adapter_id)) < len(self.task_sizes):
                if 'special_adapter_v1' in self.mode:
                    ...
                    getattr(self, adapter_id).append(Channel_Adapter_v1(c).cuda())
                    self._logger.info('Created task-specific adapter (channel v1) before layer-{}: {}=>{}'.format(adapter_id, c, c))
                elif 'channel_adapter_v2' in self.mode:
                    getattr(self, adapter_id).append(Channel_Adapter_v2(c).cuda())
                    self._logger.info('Created task-specific adapter (channel v2) before layer-{}: {}=>{}'.format(adapter_id, c, c))
                elif 'vit_adapter' in self.mode:
                    getattr(self, adapter_id).append(ViT_Adapter(c, self.hidden_dim).cuda())
                    self._logger.info('Created task-specific adapter (vit adapter) before layer-{}: {}=>{}=>{}'.format(adapter_id, c, self.hidden_dim, c))
                else:
                    raise ValueError('Unknown adapter type in mode: {}'.format(self.mode))
    ...
    def update_fc(self, nb_classes):
        ...
        self.seperate_fc.append(self.generate_fc(self.feature_dim, new_task_size+self.addition_dim))
        self._logger.info('Created seperated classifier head: {} => {}'.format(self.feature_dim, new_task_size+self.addition_dim))
    
    def load_weight_for_task0(self):
        nb_output = self.aux_fc.out_features
        weight = copy.deepcopy(self.aux_fc.weight.data)
        bias = copy.deepcopy(self.aux_fc.bias.data)
        self.seperate_fc[0].weight.data[:nb_output] = weight
        self.seperate_fc[0].bias.data[:nb_output] = bias
        self._logger.info('Updated task0 seperated classifier head with aux classifier')
