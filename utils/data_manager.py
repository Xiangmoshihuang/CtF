...
class DataManager(object):
    def __init__(self, logger, dataset_name, img_size, split_dataset:bool, shuffle=False, seed=1993, init_cls=None, increment=None, use_valid=False):
        """
        dataset_name: 数据集名称
        ...
        self._classes_name = idata.classes_name
        self._prompts = idata.templates
        ...
        self._classes_name = self._map_new_class_name(self._classes_name, self._class_order)   # class_name mapping
        # Map class_to_idx
    ...
    def _map_new_class_name(slef, cls_name, order):
        """
            数据集的类别名称，重新映射
        """
        new_cls_name = [cls_name[x] for x in order]
        return np.array(new_cls_name)
    ...
    def get_dataset(self, source, mode, indices=[], appendent=None, ret_data=False, two_view=False):
        """
        作用: 获取指定类别范围的数据
        ...
        if len(indices) > 0:
            self._logger.info('getting {}-{} , {} classes data'.format(indices[0], indices[-1], mode))
        else:
        ...
        if appendent is not None:  #len(appendent) != 0:
    ...
    def _select(self, x, y, low_range, high_range, soft_y=None, train_sample_num:int=None):
        """
