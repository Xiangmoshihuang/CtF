...
def accuracy(y_pred, y_true, nb_old, increment):
    ...
    for cur_classes in increment:
        ...
        task_acc_list.append(np.around((y_pred[idxes] == y_true[idxes]).sum()*100 / (len(idxes)+EPSILON), decimals=2))
...
def cal_class_avg_acc(y_pred, y_true):
    ...
    for class_id in np.unique(y_true):
        ...
        class_acc_list.append((y_pred[class_mask]==class_id).sum() / (len(class_mask)+EPSILON))
...
def cal_mean_class_recall(y_pred, y_true, task_size=None):
    ...
    mcr = np.around((right_of_class*100 / (num_of_class+1e-8)).sum() / (task_size+EPSILON), decimals=2)
    return mcr
